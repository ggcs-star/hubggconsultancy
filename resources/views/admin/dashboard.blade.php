@php
    $leaderName = $contestProgress?->leader->name ?? null;
@endphp

<x-layout title="Dashboard" subtitle="Overview across your entire sales team">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <div class="card p-5">
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                    ...
                    <p class="truncate text-lg font-extrabold text-slate-800">{{ $topPerformers->first()->user->name ?? '—' }}</p>
                </div>
            </div>
            <p class="mt-2 text-xs text-slate-400">{{ $topPerformers->isNotEmpty() ? number_format($topPerformers->first()->points) . ' pts' : 'No data yet' }}</p>
            <a href="{{ route('admin.leaderboard.index') }}" class="mt-3 flex items-center gap-1 text-xs font-semibold text-brand-700 hover:text-brand-800">
        ...
        <div class="card p-5">
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                    <p class="text-xs text-slate-400">Team Training Progress</p>
            ...
            <a href="{{ route('admin.learning-progress.index') }}" class="mt-3 flex items-center gap-1 text-xs font-semibold text-brand-700 hover:text-brand-800">
                View Progress <x-icon name="chevron-right" class="h-3 w-3" />
            </a>
        </div>
        ...
        <div class="card p-5">
            ...
            @endif
            <a href="{{ route('admin.leads.index') }}" class="mt-3 flex items-center gap-1 text-xs font-semibold text-brand-700 hover:text-brand-800">
        ...
        <div class="card p-5">
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                    <p class="text-xs text-slate-400">Payouts (This Month)</p>
                    <p class="text-xl font-extrabold text-slate-800">₹{{ number_format($leadStats->payouts_this_month, 0) }}</p>
                </div>
            </div>
            @if (! is_null($leadStats->payouts_change_percent))
                <p class="mt-2 text-xs font-medium {{ $leadStats->payouts_change_percent >= 0 ? 'text-emerald-600' : 'text-red-600' }}">
                    {{ $leadStats->payouts_change_percent >= 0 ? '↑' : '↓' }} {{ abs($leadStats->payouts_change_percent) }}% this month
                </p>
            @endif
            <a href="{{ route('admin.incentives.index') }}" class="mt-3 flex items-center gap-1 text-xs font-semibold text-brand-700 hover:text-brand-800">
                View Incentives <x-icon name="chevron-right" class="h-3 w-3" />
            </a>
        </div>
    </div>
    ...
    <div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="card p-5 lg:col-span-2">
            ...
            <div class="grid grid-cols-3 gap-3 sm:grid-cols-6">
                @foreach ([
                    ['icon' => 'plus', 'label' => 'Add Lead', 'route' => route('admin.leads.create')],
                    ['icon' => 'academic-cap', 'label' => 'Manage Training', 'route' => route('admin.courses.index')],
                    ['icon' => 'gift', 'label' => 'Add Contest', 'route' => route('admin.contests.create')],
                    ['icon' => 'bell', 'label' => 'Post Announcement', 'route' => route('admin.announcements.index')],
                    ['icon' => 'trending-up', 'label' => 'Learning Progress', 'route' => route('admin.learning-progress.index')],
                    ['icon' => 'help-circle', 'label' => 'Support Tickets', 'route' => route('admin.support.tickets.index')],
                ] as $action)
    ...
    <div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="card p-5">
            <div class="flex items-center justify-between">
                <p class="font-bold text-slate-800">Contest Overview</p>
                <a href="{{ route('admin.contest-tracker.index') }}" class="flex items-center gap-1 text-xs font-semibold text-brand-700 hover:text-brand-800">View All <x-icon name="chevron-right" class="h-3 w-3" /></a>
            </div>
            ...
            @if ($contestProgress)
                ...
                <div class="mt-4 space-y-3 text-sm">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="font-semibold text-slate-800">{{ $contestProgress->total_participants }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-slate-500">Current Leader</span>
                        <span class="font-semibold text-slate-800">{{ $leaderName ?? '—' }}</span>
                    </div>
                    @if ($leaderName)
                        <div class="flex items-center justify-between">
                            <span class="text-slate-500">Leader's Progress</span>
                            <span class="font-semibold text-brand-700">{{ $contestProgress->leader->progress_percent }}%</span>
                        </div>
                    @endif
                </div>
            @else
                <p class="mt-6 text-center text-sm text-slate-400">No active contests right now.</p>
            @endif
        </div>
        ...
        <div class="card p-5">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('admin.events.index') }}" class="flex items-center gap-1 text-xs font-semibold text-brand-700 hover:text-brand-800">Manage <x-icon name="chevron-right" class="h-3 w-3" /></a>
            </div>
    ...
    {{-- Announcements | Team Learning Progress | Top Performers --}}
    <div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div class="card p-5">
            <div class="flex items-center justify-between">
                <p class="font-bold text-slate-800">Latest Announcements</p>
                <a href="{{ route('admin.announcements.index') }}" class="flex items-center gap-1 text-xs font-semibold text-brand-700 hover:text-brand-800">Manage <x-icon name="chevron-right" class="h-3 w-3" /></a>
            </div>
        ...
        <div class="card p-5">
            <div class="flex items-center justify-between">
                <p class="font-bold text-slate-800">Team Learning Progress</p>
                <a href="{{ route('admin.learning-progress.index') }}" class="flex items-center gap-1 text-xs font-semibold text-brand-700 hover:text-brand-800">View All <x-icon name="chevron-right" class="h-3 w-3" /></a>
            </div>
        ...
        <div class="card p-5">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('admin.leaderboard.index') }}" class="flex items-center gap-1 text-xs font-semibold text-brand-700 hover:text-brand-800">View Leaderboard <x-icon name="chevron-right" class="h-3 w-3" /></a>
            </div>

            @if ($topPerformers->isEmpty())
                <p class="mt-6 text-center text-sm text-slate-400">No performance data yet.</p>
            @else
                <div class="mt-3 space-y-2.5">
                    @foreach ($topPerformers as $row)
                        <div class="flex items-center justify-between gap-3">
                            <div class="flex items-center gap-2.5">
                                <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold {{ $row->rank === 1 ? 'bg-amber-400 text-white' : ($row->rank === 2 ? 'bg-slate-300 text-white' : ($row->rank === 3 ? 'bg-orange-300 text-white' : 'bg-slate-100 text-slate-500')) }}">{{ $row->rank }}</span>
