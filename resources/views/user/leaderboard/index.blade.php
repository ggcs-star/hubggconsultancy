                ...
                @else
                    <div class="mt-8 space-y-3">
                        @foreach ($ranked->take(10) as $participant)
                            <div class="flex items-center justify-between gap-4 rounded-xl border border-slate-100 px-5 py-4 {{ $participant->id === auth()->id() ? 'bg-brand-50/60 border-brand-200' : ($participant->rank <= 3 ? 'bg-brand-50/40' : '') }}">
                                <div class="flex items-center gap-4">
                                    <span class="text-2xl">{{ $medals[$participant->rank - 1] ?? $participant->rank . '.' }}</span>
                                    <div>
                                        <p class="font-bold text-slate-800">{{ $participant->name }}{{ $participant->id === auth()->id() ? ' (You)' : '' }}</p>
                                    </div>
                                </div>
