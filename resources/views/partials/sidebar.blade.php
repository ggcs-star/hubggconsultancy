...
>
    <div class="flex h-20 items-center gap-3 px-6">
        ...
        <div class="leading-tight">
            <p class="text-base font-bold text-slate-800">Pre Sales School</p>
            <p class="text-xs text-slate-400">{{ $isAdmin ? 'Admin Panel' : 'Sales Team' }}</p>
        </div>
    </div>
    ...
    <nav class="flex-1 space-y-6 overflow-y-auto px-4 pb-6">
        ...
        <div>
            <p class="px-3 pb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">{{ $isAdmin ? 'Content' : 'Learning' }}</p>
            <div class="space-y-1">
        ...
        @if ($isAdmin)
            <div>
                <p class="px-3 pb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">Operations</p>
                <div class="space-y-1">
    ...
    <div class="border-t border-slate-100 p-4">
        <div class="flex items-center gap-3 rounded-xl bg-slate-50 p-3">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="rounded-lg p-2 text-slate-400 transition hover:bg-white hover:text-brand-700" title="Logout">
